...
namespace Client.SkillTrigger
{
    ...
    internal sealed class ColliderTriggerInfo
    {
        ...
        public void Load(List<ISyntaxComponent> statements)
        {
        ...
        private void LoadSceneColliderConfig(CallData stCall)
        {
            if (stCall.GetParamNum() >= 2)
            {
                m_ColliderType = ColliderType.kSceneCollider;
                m_Prefab = stCall.GetParamId(0);
                CallData param1 = stCall.GetParam(1) as CallData;
                if (null != param1)
                    m_Position = ScriptableDataUtility.CalcVector3(param1);
            }
        }

        private void LoadBoneColliderConfig(CallData stCall)
        {
        ...
        private void LoadSceneBoxColliderConfig(CallData stCall)
        {
            ...
            if (stCall.GetParamNum() >= 5)
            {
                ...
                m_Position = ScriptableDataUtility.CalcVector3(stCall.GetParam(1) as CallData);
                m_Eular = ScriptableDataUtility.CalcEularRotation(stCall.GetParam(2) as CallData);
                m_IsAttach = bool.Parse(stCall.GetParamId(3));
                m_IsShow = bool.Parse(stCall.GetParamId(4));
            }
        }

        private void LoadBoneBoxColliderConfig(CallData stCall)
        {
        ...
        private void LoadAttachEnemyConfig(CallData stCall)
        {
            if (stCall.GetParamNum() >= 8)
            {
                m_AttachConfig.IsAttachEnemy = true;
                m_AttachConfig.AttachNodeName = stCall.GetParamId(0);
                m_AttachConfig.AttachRotation = ScriptableDataUtility.CalcVector3(stCall.GetParam(1) as CallData);
